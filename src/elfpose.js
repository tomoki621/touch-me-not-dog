...
export function poseWeights(st){
  ...
  // 踏み込みと腰。**腕より先に動く**。人が斬るとき、力は足→腰→肩→腕の順に伝わる。
  // 全部を同じ重みで動かすと、体の部品が一斉に動いてぎこちなくなる（動きが機械に
  // 見えるのはたいていこれ）。ここだけ 0.06 早く立ち上げ、抜けは腕と一緒に戻す。
  const hip = ease(seg(0.34, 0.50)) * (1 - ease(seg(0.58, 1.00)));
  // 両手持ちは立ち姿から効かせる。構えが中段になって柄が体の前へ来たので、
  // 振っていないときでも左手は届く。前は振っているあいだだけ柄へ寄せていて、
  // 立ち姿では剣を右手一本でぶら下げていた。守備だけは左手で刀身を支える別の
  // 形なので、そのぶん譲る。
  return { g: st.guard, m: on * pass, w: on * wind, c: on * chop, hip: on * hip,
           two: 1 - st.guard };
}
...
// 腰が先に回り、胸が追う。s0（腰）は hip、s2（胸）は c に寄せてある。同じ重みで
// 一斉に回すと、上半身が一枚板に見える。ためでは逆に胸から捻り戻す。
export const twistOf = (p) => ({
  s0: 0.11 * p.w + 0.02 * p.m - 0.18 * (p.hip || 0) - 0.04 * p.c,
  s1: 0.20 * p.w + 0.03 * p.m - 0.10 * (p.hip || 0) - 0.13 * p.c,
  s2: 0.15 * p.w + 0.02 * p.m - 0.02 * (p.hip || 0) - 0.18 * p.c,
});
...
export const TRUNK = {
  ...
  // 斬るときは右足で踏み込む。腿は負で前、膝は正で畳む（走りのクリップで実測）。
  // 前は 0.22 しか出しておらず、腕だけ振って足は揃ったままだった。
  RightUpLeg: [[1, 0, 0, { g:  0.12, hip: -0.52, w: -0.16 }]],
  RightLeg:   [[1, 0, 0, { g:  1.88, hip:  0.34, w: 0.26 }]],
  RightFoot:  [[1, 0, 0, { g:  0.78, hip: -0.14 }]],
  // 左脚：腿を前へ倒してひざを立て、脛を垂直に戻して足の裏を地につける。
  // 少し外へ開くと、体の前に剣を通す隙間ができる。
  // 斬り抜きでは前足。踏み込んでひざを曲げる。ここが折れないと、腕だけ振って
  // 体は棒立ちに見える。
  // 左脚は後ろ足。踏み込みで後ろへ残し、膝を伸ばして地面を蹴る。かかとは浮く。
  LeftUpLeg:  [[1, 0, 0, { g: -1.42, hip:  0.44, w: -0.20 }], [0, 0, 1, { g: 0.20 }]],
  LeftLeg:    [[1, 0, 0, { g:  1.38, hip:  0.10, w: 0.30 }]],
  LeftFoot:   [[1, 0, 0, { g:  0.06, hip: -0.34 }]],

  // 胴。X まわりは正で前へ屈み、負で反る（描き出して確かめた）。守備は剣の陰へ
  // 深く屈み、打ち抜きも前へ入る。ただし屈めすぎるとひざ立ちの形が潰れて、
  // うずくまって見える。腰から順に少しずつ折るのが、いちばん潰れない。
  Spine:   [[1, 0, 0, { g:  0.30, hip: 0.16, c:  0.18, w: -0.12 }], [0, 1, 0, { tw0: 1 }]],
  Spine01: [[1, 0, 0, { g:  0.20, hip: 0.10, c:  0.26, w: -0.10 }], [0, 1, 0, { tw1: 1 }]],
  Spine02: [[1, 0, 0, { g:  0.12, hip: 0.06, c:  0.32, w: -0.08 }], [0, 1, 0, { tw2: 1 }]],

  // 肩。開くのは肩から。腕の骨だけを大きくひねると関節が外れて見える。
  // 守備では両肩をすぼめる。首をうずめる形になり、頭を下げたのが効いてくる。
  RightShoulder: [[0, 0, 1, { g: -0.24, w: -0.30, c: 0.22 }],
                  [1, 0, 0, { hip: 0.10, c: 0.24 }]],
  LeftShoulder:  [[0, 0, 1, { g:  0.24, w:  0.10, c: -0.12 }],
                  [1, 0, 0, { g: -0.22, c: -0.24 }]],
...
// ---------------------------------------------------------------- 体そのもの
// 骨ではなく入れ物（chara）を動かす分。前はこれだけ src/elf.js の中にあって、
// tools/elfpose.mjs には載っていなかった。だから描き出しでは前へ踏み込んで
// 見えるのに、実機では上体だけ後ろへ反っていた。**見た絵が画面と違う**という、
// この表を作った目的そのものを外していた。ここへ移して、両方から呼ぶ。
//
// pitch は正が前傾（chara は素の Group なので three の定義そのまま。rotation.x が
// 正だと上ベクトルが +Z＝正面側へ倒れる）。ここが負のままで、踏み込みながら
// のけぞって いた。
// 入れ物の原点は足元なので、ここで回すと脚ごと傾く。大きく倒すと前の足が床を
// 突き抜ける（0.42 で最下点が -0.32 まで潜った）。だから **体ごとの傾きは浅く**
// して、前へ折る分の大半は背骨の c に持たせてある。脚は動かない。
// 残る浅い傾きも、足元ではなく腰の高さを軸にする。原点回りに回したあと、腰が
// 元の位置へ戻るぶんだけ寄せ直せば、腰を軸に回したのと同じになる。
const HIP_Y = 1.05;   // 腰のおよその高さ（背丈 1.9 のとき）

export function bodySwing(p){
  const w = p.w || 0, c = p.c || 0, hip = p.hip || 0;
  const pitch = -0.10 * w + 0.18 * c;    // ためで軽く反り、斬りで前へ入る
  return {
    pitch,
    dz: 0.30 * hip + 0.06 * c - 0.10 * w - HIP_Y * Math.sin(pitch),
    // 踏み込みでは沈めない。脚を開くぶん足はすでに下がる（開いたまま腰まで
    // 落とすと、前足が床を突き抜けた）。ためだけ軽く沈める。
    dy: -0.05 * w + HIP_Y * (1 - Math.cos(pitch)),
  };
}
...
export const DIR = {
  // 右腕。素は中段の構え。肘を折って柄を体の前へ持ってくる（腕を下ろしたまま
  // だと、剣が右足の横にぶら下がって両手で持てない）。そこから右肩の上へ
  // 振りかぶり、相手の左肩口へ斜めに斬り下ろす。真横へ薙ぐと払っただけに
  // 見えるので、終点は自分の左の腰より下まで落とす。
  RightArm:     { rest: [-0.02, -0.90,  0.44],
                  wind: [-0.66,  0.58, -0.48],
                  pass: [-0.24,  0.52,  0.82],
                  chop: [ 0.34, -0.62,  0.71],
                  guard:[-0.14, -0.36,  0.92] },
  // 前腕。ためで肩ごしに折り畳み、斬り下ろしで伸ばしきる。ここが肘の見せ場。
  RightForeArm: { rest: [ 0.74, -0.20,  0.64],
                  wind: [-0.05,  0.05, -1.00],
                  pass: [-0.06,  0.60,  0.80],
                  chop: [ 0.44, -0.68,  0.59],
...
export const SWORD = {
  rest:  [ 0.02,  0.44,  0.90],   // 構え：中段。刃先を正面のやや上へ。両手で柄を持つ。
...
function twoHand(ctx, p, charaQ, handQ){
  ...
  // 肩を少し送る。ただし **届いていないときだけ**。
  //
  // 前は届いていても毎フレーム 55% 送っていた。立ち姿では柄が体の前にあって
  // 楽に届くのに肩を内へ回し続けるので、左肩が前へ入って潰れて見えた（実機で
  // 三角筋が消えていた）。肩は腕が足りないぶんの足しであって、常時効かせる
  // ものではない。腕の長さの 88% を超えた分だけ送る。
  if (bones.LeftShoulder){
    _d2.subVectors(_T, _S);
    const need = _d2.length();
    const reach = (L1 + L2) * 0.88;
    const over = reach > 1e-6 ? Math.min(1, Math.max(0, (need - reach) / (reach * 0.35))) : 0;
    if (over > 0.001 && _d2.lengthSq() > 1e-9){
      _d2.normalize();
      _qKeep.copy(bones.LeftShoulder.quaternion);
      aimBoneW(bones, 'LeftShoulder', 'LeftArm', _d2);
      bones.LeftShoulder.quaternion.slerp(_qKeep, 1 - TWO_SHOULDER * t * over);
