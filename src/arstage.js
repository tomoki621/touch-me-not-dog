...
// 【いまは「映っていた」値に戻してある】0.38 / 0.72 は収まりとしては正しかったが、
// その版で実機からキャラが消えた。三角形は 71301 描かれ、箱もカメラも正しく、
// それでも画面に出ない。原因が分かるまで、映ることが確かめられている値に置く。
//   0.485 → 倍率ちょうど 1.0（背丈 1.9 / 視野の高さ 3.917）
//   0.50  → 足元が画面のまん中、つまり位置 (0,0,-DIST)
// 収まりは悪い（体が上半分を占め、頭が画面の外へ抜ける）。承知のうえで戻す。
const FLAT_FILL = 0.485;
const FLAT_FOOT = 0.50;

const _camP = new THREE.Vector3();

// ?cam を付けて開いたときだけ、生のカメラ映像へ触れる機能を「任意で」頼む。
//
// 【?dbg とは別にする】前は ?dbg でこれを頼んでいた。camera-access を頼むと
// ARCore は別のカメラ構成に切り替わる。その結果、?dbg を付けたとたんに面が
// 見つからなくなり、輪が出なくなった。**調べるための計器が、調べたい対象を
// 壊していた。** ?dbg は何も足さない・何も変えない、ただ見るだけに戻す。
