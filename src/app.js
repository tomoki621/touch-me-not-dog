...
function update(){
  ...
  if (rigged && !clipDriven){
    ...
    boneSet('Spine',   [[1,0,0, -g*0.28 + r*0.14], [0,1,0, 0.05*w - 0.07*c]]);
    boneSet('Spine01', [[0,1,0, 0.10*w - 0.14*c], [1,0,0, -g*0.16 - c*0.12]]);
    boneSet('Spine02', [[0,1,0, 0.08*w - 0.11*c], [1,0,0, r*0.30 - c*0.18 - g*0.08]]);
    ...
    // 右腕。斜め上へ振りかぶり、正面へ振り下ろす。
    _dA.set(-0.40, -0.90,  0.06);           // 構え
    _dB.set(-0.60,  0.70, -0.32);           // ため：右斜め上へ振りかぶる
    _dC.set( 0.06, -0.56,  0.83);           // 振り下ろし：正面やや下へ
    ...
    _dB.set(-0.44,  0.46, -0.77);           // 頭の右後ろへ畳む
    _dC.set( 0.18, -0.62,  0.76);           // 正面下へ伸ばしきる
    _dir.copy(_dA).lerp(_dB, w).lerp(_dC, c);
    ...
    if (bones.RightHand){
      ...
      _dB.set(-0.54,  0.74, -0.40);           // ため：右斜め上へ立てる
      _dC.set( 0.18, -0.60,  0.78);           // 振り下ろし：正面下へ
      _dir.copy(_dA).lerp(_dB, w).lerp(_dC, c);
